...
```bash
# Qodana 官方 CLI：同 PyCharm 引擎，社区版 linter，CI 标准。
# 首次需 Docker 或拉取 linter 运行时（仍分钟级，结果入缓存后快）；后续用缓存。
qodana scan --linter jetbrains/qodana-python-community --within-docker false
```
