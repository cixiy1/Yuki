...
- 工具闭环有轮次上限（`max_tool_rounds`）：整轮内出现重复工具调用时，会临时加载可用外置包并把具体工具放回工具列表，提示模型直接调用；仍重复才停止循环，注入“本回合实际执行过的工具”清单并强制直接回答（禁止编造未执行工具的结果），同时发出 `warning` 事件，避免死循环与虚假工具结果。
...
- `scan_packages()` 返回 `PackageScan`（`packages` 为发现的包，`available` 为白名单后可用包，`skipped` 为跳过原因），并同时存入 `registry.package_scan`。
